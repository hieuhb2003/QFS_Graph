...
def find_vdb_entry_pairs(
    vdb1: Dict[str, Any], 
    vdb2: Dict[str, Any], 
    threshold: float = 0.8, 
    max_threshold: float = None,
    batch_size: int = 50000  # Add batch_size parameter
) -> List[Tuple[Dict[str, Any], Dict[str, Any], float]]:
    """Batch-processed version to handle large matrices."""
    try:
        ...
        if matrix1.shape[0] != len(vdb1["data"]) or matrix2.shape[0] != len(vdb2["data"]):
            raise ValueError("Matrix dimensions mismatch with data length")

        # Pre-normalize matrix2 once
        matrix2_norm = normalize(matrix2)
        pairs = []

        # Process matrix1 in batches
        for batch_start in trange(0, matrix1.shape[0], batch_size):
            batch_end = min(batch_start + batch_size, matrix1.shape[0])
            batch = matrix1[batch_start:batch_end]
            
            # Normalize current batch
            batch_norm = normalize(batch)
            
            # Compute batch similarity
            batch_similarity = np.dot(batch_norm, matrix2_norm.T)
            
            # Find matches in current batch
            if max_threshold is None:
                matches = np.where(batch_similarity >= threshold)
            else:
                matches = np.where(
                    (batch_similarity >= threshold) & 
                    (batch_similarity < max_threshold)
                )
            
            # Convert batch indices to original indices
            rows = matches[0] + batch_start
            cols = matches[1]
            scores = batch_similarity[matches]

            # Collect pairs with original data entries
            for row, col, score in zip(rows, cols, scores):
                try:
                    ...
                    logger.error(f"Index error at ({row}, {col}): {e}")

        logger.info(f"Found {len(pairs)} pairs using batch processing")
        return pairs
...
#     gpu_id: int = 0  # GPU ID để sử dụng
# ) -> List[Tuple[Dict[str, Any], Dict[str, Any], float]]:
#     """Phiên bản sử dụng GPU để xử lý các ma trận lớn."""
#     try:
#         import faiss
...
#         if ngpus == 0:
#             logger.warning("No GPUs available. Falling back to CPU implementation.")
#             return find_vdb_entry_pairs_optimized(vdb1, vdb2, threshold, max_threshold, batch_size, use_faiss=True)
        
#         logger.info(f"Using GPU {gpu_id} out of {ngpus} available GPUs")
...
#         # Chuyển đổi dữ liệu
#         matrix1 = buffer_string_to_array(vdb1["matrix"]).reshape(-1, embedding_dim).astype(np.float32)
#         matrix2 = buffer_string_to_array(vdb2["matrix"]).reshape(-1, embedding_dim).astype(np.float32)
...
#         # Tạo index trên CPU
#         cpu_index = faiss.IndexFlatIP(embedding_dim)  # Inner product ~ cosine similarity for normalized vectors
        
#         # Chuyển index vào GPU
#         res = faiss.StandardGpuResources()  # Cấp phát tài nguyên GPU
#         gpu_index = faiss.index_cpu_to_gpu(res, gpu_id, cpu_index)
        
#         # Thêm dữ liệu vào index
#         gpu_index.add(matrix2_norm)
#         logger.info(f"Added {matrix2_norm.shape[0]} vectors to GPU index")
        
#         pairs = []
#         total_pairs = 0
        
#         # Xử lý matrix1 theo batch để tránh vượt quá VRAM
#         for batch_start in trange(0, matrix1_norm.shape[0], batch_size, desc="GPU Search"):
#             batch_end = min(batch_start + batch_size, matrix1_norm.shape[0])
#             batch = matrix1_norm[batch_start:batch_end]
            
#             # Số lượng kết quả trả về cho mỗi query vector
#             k = min(500, matrix2_norm.shape[0])  # Giới hạn số lượng kết quả
            
#             # Tìm kiếm trên GPU
#             D, I = gpu_index.search(batch, k)  # D là khoảng cách (điểm số), I là chỉ số
...
#                 for j in range(k):
#                     if I[i, j] < 0:  # Chỉ số không hợp lệ, thường là khi kết quả ít hơn k
#                         continue
                    
#                     score = float(D[i, j])
#                     col_idx = int(I[i, j])
                    
#                     if score >= threshold and (max_threshold is None or score < max_threshold):
#                         batch_pairs.append((row_idx, col_idx, score))
            
#             # Chuyển đổi các chỉ số thành dữ liệu thực tế
#             for row, col, score in batch_pairs:
#                 try:
#                     pairs.append((
#                         vdb1["data"][row],
#                         vdb2["data"][col],
...
#             logger.info(f"Processed batch {batch_start}-{batch_end}, found {len(batch_pairs)} pairs, total: {total_pairs}")
        
#         logger.info(f"Found total of {len(pairs)} pairs using GPU-accelerated search")
#         return pairs

#     except ImportError:
#         logger.error("FAISS GPU not available. Please install with: pip install faiss-gpu")
